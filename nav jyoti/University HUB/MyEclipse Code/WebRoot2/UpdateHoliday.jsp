...
<html>
	...
	<body>
		...
		<form method='post'
			action="<%=request.getContextPath() + "/UpdateHolidays"%>"
			name="holiday">
			<table cellspacing=2 align="center" bgcolor="27C2D0">
				<c:forEach var="company" items="${holiday}">
					<tr>
						<input type='hidden' name="hoiidayId"
							value='${company.hoiidayId }'>
						<td align='right'>
							<b>HolidayName :</b>
						</td>
						<td>
							<input type='text' name='holidayName'
								value="${company.holidayName }">
						</td>
						<tr>
							<td align='right'>
								<b>Date of Holiday :</b>
							...
							<td>
								<input type="text" name="holidaydate"
									value="${company.holidaydate}" size="20" readonly="readonly" />
								<a
							...
							<tr>
								...
								<td>
									<select name="day" onChange="cleartext()">
										<option value="${company.day}" selected="true">
											<font size="3" face="Verdana">${company.day} </font>
										</option>
										<option value="Sunday">
